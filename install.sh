...
apply_fixes() {
    ...
    # Fix 2: CMakeLists.txt SM100/SM120 MOE kernels (idempotent)
    log_info "Ensuring CMakeLists.txt SM100/SM120 arch fixes..."
    # Always attempt both replacements; they are no-ops if already applied
    ...
    if grep -q 'cuda_archs_loose_intersection(SCALED_MM_ARCHS "10.0f;11.0f;12.0f"' CMakeLists.txt \
       && grep -q 'cuda_archs_loose_intersection(SCALED_MM_ARCHS "10.0a;12.1a"' CMakeLists.txt; then
        log_success "CMakeLists.txt SM100/SM120 arch fixes present"
    else
        log_warning "CMakeLists.txt SM100/SM120 arch fixes may not have applied; please check manually"
    fi
